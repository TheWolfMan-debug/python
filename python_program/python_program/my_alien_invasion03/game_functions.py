...
def check_events(ai_settings, screen, stats, sb, play_button, ship, aliens, bullets, my_music):
    """响应按键和鼠标事件"""
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
        ...
        elif event.type == pygame.KEYDOWN or pygame.key.set_repeat(10, 75):
            check_keydown_events(
                event, ai_settings, screen, stats,  sb, ship, aliens, play_button, bullets, my_music)
            ...
            check_play_button(ai_settings, screen, stats, sb,
                              play_button, ship, aliens, bullets, mouse_x, mouse_y, my_music)

        # elif event.type == VIDEORESIZE:
        #     # get the size of the window
        #     SCREEN_SIZE = event.size
        #     # set the mode of the window
        #     myscreen = pygame.display.set_mode(SCREEN_SIZE, RESIZABLE, 32)
  

def check_play_button(ai_settings, screen, stats, sb, play_button, ship, aliens, bullets, mouse_x, mouse_y, my_music):
    """在玩家单击Play按钮式开始新游戏"""
    ...
    if not stats.game_active:
        ...
        if game_button_clicked:
            initialize_game_settings(
                ai_settings, stats, sb, aliens, bullets, screen, ship, my_music)
        # 点击开始新游戏
...
def help_document(stats):
    ...
    f_open = open(filename,encoding = 'utf-8')
    content = f_open.read()
    f_open.close()
    # 第4步，在图形界面上设定标签
...
def initialize_game_settings(ai_settings, stats, sb, aliens, bullets, screen, ship, my_music):
    # 重置游戏设置
    ...
    # 停止播放主菜单音乐
    my_music.menu_music_stop()
    # 开始播放战斗音乐
    if ai_settings.music_flag == 1:
        my_music.game_music()


def check_keydown_events(event, ai_settings, screen, stats,  sb, ship, aliens, play_button, bullets, my_music):
    if event.key == pygame.K_RIGHT:
        ...
        if ai_settings.music_flag == 1:
            pygame.mixer.pause()
            ai_settings.music_flag *= -1
        elif ai_settings.music_flag == -1:
        ...
        if not ai_settings.ship_invincible:
            ai_settings.ship_invincible = True
        elif ai_settings.ship_invincible:
            ai_settings.ship_invincible = False
    # elif event.key == pygame.K_f:
    #     SCREEN_SIZE = event.size
    #     ai_settings.fullscreen = not ai_settings.fullscreen
    #     if not ai_settings.fullscreen:
...
def check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship, aliens, bullets, my_music):
    # 检查是否有子弹击中了敌人
    # 如果是这样，就删除相应的子弹和外星人
...
def ship_hit(ai_settings, screen, stats, sb, ship, aliens, bullets, my_music):
    """响应被外星人撞到的"""
...
def update_aliens(ai_settings, screen, stats, sb, ship, aliens, bullets, my_music):
    ...
    if not ai_settings.ship_invincible:
        if pygame.sprite.spritecollideany(ship, aliens):
            ...
            ship_hit(ai_settings, screen, stats, sb,
                     ship, aliens, bullets, my_music)


def update_bullets(ai_settings, screen, stats, sb, ship, aliens, bullets, my_music):
    # 更新子弹状态
    bullets.update()
    ...
    check_bullet_alien_collisions(
        ai_settings, screen, stats, sb, ship, aliens, bullets, my_music)
